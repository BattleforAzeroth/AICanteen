...
int Yolo::draw(cv::Mat &rgb, const std::vector<Object> &objects) {
    static const char *class_names[] = {
            "Beijing Beef", "Black Pepper Angus Steak", "Broccoli Beef",
            "Chow Mein", "Cream Cheese Rangoon", "Egg Roll", "Eggplant Tofu",
            "Fried Dumpling", "Fried Rice", "Grilled Teriyaki Chicken",
            "Honey Sesame Chicken Breast", "Honey Walnut Shrimp", "Kung Pao Chicken",
            "Mushroom Chicken", "String Bean Chicken Breast", "Super Greens",
            "SweetFire Chicken Breast", "The Original Orange Chicken", "Veggie Spring Roll",
            "White Steamed Rice", "Wok-Fired Shrimp"
    };
...
int *Yolo::getLabels(const std::vector<Object> &objects) {
    int len = objects.size();
    int *labels = new int[len + 1];
    labels[0] = len;
    if (len != 0) {
        int k = 0;
        for (const auto &object: objects) {
            labels[++k] = object.label;
        }
    }
    return labels;
}
